...
#define UPDATE_URL    "http://bitbucket.toastdev.de/sourcemod-plugins/raw/master/hideseek.txt"
public Plugin:myinfo = 
{
	name = "HideSeekDay",
	author = "Draakoor",
	description = "A HideSeekDay plugin for Jail",
	version = "1.0.4",
...
new hideseek[MAXPLAYERS +1];
new R;
new G;
new B;
new bool:colors = true;
new bool:beacon = false;
new hideseekRound[MAXPLAYERS + 1];
new g_BeamSprite = -1;
new g_HaloSprite = -1;
new g_Game = 0;
public OnMapStart()
...
public OnPluginStart()
{
	...
	CreateConVar("hideseek_version", "1.0", "The current version of the plugin");
	c_fd_R = CreateConVar("hideseek_R", "255", "The Red Color for marking");
	c_fd_G = CreateConVar("hideseek_G", "0", "The Green Color for marking");
	c_fd_B = CreateConVar("hideseek_B", "0", "The Blue Color for marking");
	g_color = CreateConVar("hideseek_enable_color", "1", "Enable colored marking for HideSeekDay players");
	g_beacon = CreateConVar("hideseek_enable_beacon", "0", "Enable beacon marking for HideSeekDay players");
	g_beacon_interval = CreateConVar("hideseek_beacon_interval", "1.0", "The Interval for the beacon");
	g_radius = CreateConVar("hideseek_beacon_radius", "375.0", "The radius of the beacon");
	...
	RegAdminCmd("sm_sfhide", HideseekCommandHandler, FlagToBit(Admin_Kick), "Give Hideseek Day");
	
	LoadTranslations("hideseek.phrases");
...
{
   RegPluginLibrary("hideseek");
   CreateNative("Has_hideseek", Native_Has_hideseek);
   CreateNative("Set_hideseek", Native_Set_hideseek);
...
public PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	client = GetClientOfUserId(userid);
	hideseek[client] = 0;
	if(hideseekRound[client] == 1)
	{
		Markhideseek(client);
		hideseek[client] = 1;
	}
	hideseekRound[client] = 0;
}
...
public Action:HideseekCommandHandler(client, args)
{
	...
	else if(args > 0)
	{
		...
		for(new i; i <= target_count;i++){
			...
			if(IsPlayerAlive(target) && GetClientTeam(target) == 2){
				...
				hideseekRound[target] = 1;
				new String:targetname[MAX_NAME_LENGTH];
				GetClientName(target, targetname, sizeof(targetname)); 
				CPrintToChat(client, "%t %t", "prefix", "player_hideseek_next_round", targetname);
			}
		}
		return Plugin_Handled;
		...
		new Handle:menu = CreateMenu(hideseekMenuHandler);
		Format(string,sizeof(string),"%t", "hideseekMenuTitle", LANG_SERVER);
...
public hideseekMenuHandler(Handle:menu, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		...
		new String:string[64];
		new Handle:menu2 = CreateMenu(hideseekMenuHandler);
		Format(string,sizeof(string),"%t", "hideseekMenuTitle", LANG_SERVER);
...
Markhideseek(client)
{
	if(IsPlayerAlive(client))
	{
		...
		CPrintToChatAll("%t %t", "prefix", "hideseek_sucess", name);
		hideseek[client] = 1;
	}
...
public Native_Has_hideseek(Handle:plugin, numParams){
	new client = GetNativeCell(1);
	if(hideseek[client] == 1)
	{
		return true;
	}
	return false;
}
public Native_Set_hideseek(Handle:plugin, numParams){

	new client = GetNativeCell(1);
	if(IsClientInGame(client) && hideseek[client] != 1 && GetNativeCell(2) == 1){
		Markhideseek(client);
		return true;
	}
	else if(IsClientInGame(client) && GetNativeCell(2) != 1){
		hideseekRound[client] = 1;
		return true;
	}
	else{
		return false;
